...
//		Description:	False Nearest Neighbors (FNN) Algorithm
//----------------------------------------------------------------------
...
int main(int argc, char **argv)
{
	...
	dataPts = annAllocPts(maxPts, dim+1);	// allocate data points
...
//		cout << "\tNN:\tIndex\tDistance\n";
		for (int i = 0; i < k; i++) {			// print summary
			...
			float R = abs(queryPt[dim]-dataPts[nnIdx[i]][dim])/dists[i];
			cout << queryPt[dim] << '\t' << dataPts[nnIdx[i]][dim] << '\n';
			int F = 0;
			if (R > 10) {
				F = 1;
			}
			if (i==0) {
				outFile << "," << i << "," << nnIdx[i] << "," << R << "," << F <<"\n";
			}
			else {
				outFile << "NaN,NaN" << "," << i << "," << nnIdx[i] << "," << R << "," << F <<"\n";
			}
		}
	}
